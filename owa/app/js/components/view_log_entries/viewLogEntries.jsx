...
export default class LogEntries extends React.Component {
    ...
    advancedSearch() {
        const {source, identifier, comment} = this.state.searchFilters;
        const endDate = this.state.endDate.toISOString();
        const generatedBy = this.state.generatedBy;
        if (!this.state.startDate){
            apiCall(null, 'get', `/idgen/logentry?v=full&generatedBy=${generatedBy}&identifier=${identifier}&comment=${comment}&source=${source}&toDate=${endDate}`).then((response) => {
            this.setState({logEntries: response.results});
        });
        }
        else{ 
    ...
    handleChangeEnd(date) {
        this.setState({endDate: date, dateChanged: !this.state.dateChanged});
    }
    componentDidMount() {
        this.getLogEntries();
            ...
            <div className="logwrapper">
                ...
                <div className="logs_table_area">
                            ...
                            <form id="advancedSearchForm">
                                ...
                                <fieldset>
                                    ...
                                    <div className="col-sm-6 col-md-4">
                                        ...
                                        <Input value={this.state.generatedByValue} name="generatedBy" onChange={this.handleSearch}/>
                                    </div>
                                    <div className="col-sm-6 col-md-4">
                    ...
                    <div className="table-responsive logs_table">
                        { (this.state.searchFilters.source || this.state.searchFilters.identifier || this.state.generatedByValue || this.state.searchFilters.comment || this.state.startDate|| this.state.dateChanged)  &&
                        <div className="filterLabels col col-xs-12">
                            ...
                            { (this.state.generatedByValue) && <span className="filteredBy">Generated By</span>}
                            { (this.state.searchFilters.comment) && <span className="filteredBy">Comments</span>}
                            { (this.state.startDate) && <span className="filteredBy">Generated After (Date)</span>}
                            { (this.state.dateChanged) && <span className="filteredBy">Generated Before (Date)</span>}
                        </div>
                        }
                        <ReactTable
                            data={logEntries}
