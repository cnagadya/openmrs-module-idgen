 ...
 */
// generated on 2017-11-10 using generator-openmrs-owa 0.6.0
'use strict';
...
const THIS_APP_ID = 'bootstrapping';
...
var getConfig = function () {
		...
		try {
				...
				config = {
						'LOCAL_OWA_FOLDER': '/Users/wonder_woman/Desktop/Andela/OpenMRS/referenceapplication-standalone-2.6.0' +
										'/appdata\\owa/',
						'APP_ENTRY_POINT': 'http://localhost:8081/openmrs-standalone/owa/bootstrapping/index.html'
				};
...
var resolveBrowserSyncTarget = function () {
		if (targetPort != null && targetPort != 'null') {
				return config
						.APP_ENTRY_POINT
						.substr(0, 'http://localhost:'.length) + targetPort + config
						.APP_ENTRY_POINT
						.substr('http://localhost:'.length + targetPort.toString().length, config.APP_ENTRY_POINT.length);
		} else {
...
plugins.push(new CommonsChunkPlugin({name: 'vendor', filename: vendorOutputFile}));

plugins.push(new HtmlWebpackPlugin({template: './app/index.html', inject: 'body'}));
...
var webpackConfig = {
		quiet: false,
		entry: {
				app: `${__dirname}/app/js/bootstrapping`,
				css: `${__dirname}/app/css/bootstrapping.css`,
